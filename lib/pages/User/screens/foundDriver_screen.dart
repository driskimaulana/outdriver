...
class _founddriverstate extends State<foundDriver> {
  ...
  Future fetchPost() async {
    var body = {
      "latitude": widget.startPosition!.geometry!.location!.lat.toString(),
      "longitude": widget.startPosition!.geometry!.location!.lng.toString()
    };
    ...
    if (response.statusCode == 200) {
      if (jsonDecode(response.body())['data'] != null) {
        setState(() {
          driver = Driver.fromJson(jsonDecode(response.body())['data']);
        });
      }
  ...
  Future<void> createOrder() async {
    var body = {
      ...
      "fromCoords": [
        widget.startPosition!.geometry!.location!.lat.toString(),
        widget.startPosition!.geometry!.location!.lng.toString()
      ].toString(),
      "toName": widget.endPosition!.name,
      "toCoords": [
        widget.endPosition!.geometry!.location!.lat.toString(),
        widget.endPosition!.geometry!.location!.lng.toString()
      ].toString(),
      "cost": widget.total.toString(),
      "paymentMethod": "Dana"
    };
    var url = Uri.parse("${Utils.BASE_API_URL}/transaction/orderDriver");
    var token = await SessionManager().get("token");
    var response = await http.post(url, body: body, headers: {
      'Authorization': 'Bearer $token',
    });
